...
##################Set Resistance#########################
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        cnt = 0
        if not Seperation:
            raise ValueError('Digital implementation do not available for Non Sep')
        if Seperation:
                ...
                for x , y in (child.named_children()):
                    if not ( isinstance(y, nn.BatchNorm2d) or isinstance(y, nn.BatchNorm1d)):
                        for p in list(y.parameters()):
                            ...
                            if bit==1:
                                raise ValueError('Digital implementation do not available for 1-bit')
                            if bit==2:
                                linear_G11 = ( (GH_var[0] + m*GH_var[1]) - (GL_var[0] + m*GL_var[1]) )*scale; 
                                linear_G10 = ( (GH_var[0] + m*GL_var[0]) - (GL_var[1] + m*GL_var[2]) )*scale; 
                                linear_G01 = ( (GL_var[0] + m*GL_var[1]) - (GH_var[0] + m*GL_var[2]) )*scale; 
                                linear_G00 = ( (GL_var[0] + m*GL_var[1]) - (GH_var[0] + m*GH_var[1]) )*scale;
                            ...
                            if bit==3:
                                linear_G111 = ( (GH_var[0] + m*GH_var[1] + n*GH_var[2]) - (GL_var[0] + m*GL_var[1] + n*GL_var[2]) )*scale; 
                                linear_G110 = ( (GH_var[0] + m*GL_var[0] + n*GH_var[1]) - (GL_var[1] + m*GL_var[2] + n*GL_var[3]) )*scale; 
                                linear_G101 = ( (GH_var[0] + m*GH_var[1] + n*GL_var[0]) - (GL_var[1] + m*GL_var[2] + n*GL_var[3]) )*scale; 
                                linear_G100 = ( (GH_var[0] + m*GL_var[0] + n*GL_var[1]) - (GL_var[2] + m*GL_var[3] + n*GL_var[4]) )*scale;
                                linear_G011 = ( (GL_var[0] + m*GL_var[1] + n*GL_var[2]) - (GH_var[0] + m*GL_var[3] + n*GL_var[4]) )*scale; 
                                linear_G010 = ( (GL_var[0] + m*GL_var[1] + n*GL_var[2]) - (GH_var[0] + m*GH_var[1] + n*GL_var[3]) )*scale; 
                                linear_G001 = ( (GL_var[0] + m*GL_var[1] + n*GL_var[2]) - (GH_var[0] + m*GL_var[3] + n*GH_var[1]) )*scale; 
                                linear_G000 = ( (GL_var[0] + m*GL_var[1] + n*GL_var[2]) - (GH_var[0] + m*GH_var[1] + n*GH_var[2]) )*scale;
                            ...
                            if bit==4:
                                linear_G1111 = ( (GH_var[0] + m*GH_var[1] + n*GH_var[2] + o*GH_var[3]) - (GL_var[0] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1110 = ( (GH_var[0] + m*GL_var[0] + n*GH_var[1] + o*GH_var[2]) - (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1101 = ( (GH_var[0] + m*GH_var[1] + n*GL_var[0] + o*GH_var[2]) - (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1100 = ( (GH_var[0] + m*GL_var[0] + n*GL_var[1] + o*GH_var[1]) - (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) )*scale;
                                linear_G1011 = ( (GH_var[0] + m*GH_var[1] + n*GH_var[2] + o*GL_var[0]) - (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1010 = ( (GH_var[0] + m*GL_var[0] + n*GH_var[1] + o*GL_var[1]) - (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1001 = ( (GH_var[0] + m*GH_var[1] + n*GL_var[0] + o*GL_var[1]) - (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) )*scale; 
                                linear_G1000 = ( (GH_var[0] + m*GL_var[0] + n*GL_var[1] + o*GL_var[2]) - (GL_var[4] + m*GL_var[5] + n*GL_var[6] + o*GL_var[3]) )*scale;
                                linear_G0111 = ( (GL_var[4] + m*GL_var[5] + n*GL_var[6] + o*GL_var[3]) - (GH_var[0] + m*GL_var[0] + n*GL_var[1] + o*GL_var[2]) )*scale; 
                                linear_G0110 = ( (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GH_var[1] + n*GL_var[0] + o*GL_var[1]) )*scale; 
                                linear_G0101 = ( (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GL_var[0] + n*GH_var[1] + o*GL_var[1]) )*scale; 
                                linear_G0100 = ( (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GH_var[1] + n*GH_var[2] + o*GL_var[0]) )*scale;
                                linear_G0011 = ( (GL_var[4] + m*GL_var[5] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GL_var[0] + n*GL_var[1] + o*GH_var[1]) )*scale; 
                                linear_G0010 = ( (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GH_var[1] + n*GL_var[0] + o*GH_var[2]) )*scale; 
                                linear_G0001 = ( (GL_var[4] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GL_var[0] + n*GH_var[1] + o*GH_var[2]) )*scale; 
                                linear_G0000 = ( (GL_var[0] + m*GL_var[1] + n*GL_var[2] + o*GL_var[3]) - (GH_var[0] + m*GH_var[1] + n*GH_var[2] + o*GH_var[3]) )*scale;
        ...
        print(model.classifier[6].weight.data[0:5,0:5])
