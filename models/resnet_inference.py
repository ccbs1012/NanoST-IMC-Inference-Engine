...
__all__ = ['resnet_inference']

def Binaryconv3x3(in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return BinarizeConv2d_inference(in_planes, out_planes, kernel_size=3, stride=stride,
...
def init_model(model):
    for m in model.modules():
        if isinstance(m, BinarizeConv2d_inference):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = BinarizeConv2d_inference(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = BinarizeConv2d_inference(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = BinarizeConv2d_inference(planes, planes * 4, kernel_size=1, bias=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1,do_bntan=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                BinarizeConv2d_inference(self.inplanes, planes * block.expansion,
...
class ResNet_imagenet(ResNet):
    ...
    def __init__(self, num_classes=1000,
        ...
        self.inplanes = 64
        self.conv1 = BinarizeConv2d_inference(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        ...
        self.fc = BinarizeLinear_inference(512 * block.expansion, num_classes)
...
class ResNet_cifar10(ResNet):
    ...
    def __init__(self, num_classes=10,
        ...
        n = int((depth - 2) / 6)
        self.conv1 = BinarizeConv2d_inference(3, 16*self.inflate, kernel_size=3, stride=1, padding=1,
                               bias=False)
        ...
        self.fc = BinarizeLinear_inference(64*self.inflate, num_classes)
